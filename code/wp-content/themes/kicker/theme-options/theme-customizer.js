/* global kicker_color_schemes, kicker_dependencies, Color */
...
( function( api ) {
	...
	for (var i in kicker_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'kicker-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'kicker-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'kicker-vars' );
	...
	api.bind( 
		'ready', function() {
			...
			jQuery( '#customize-header-actions #save' )
				.before( '<input type="button" class="button customize-action-reset" value="' + kicker_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' )
				.on( 'click', function(e) {
					if (confirm( kicker_customizer_vars['msg_reset_confirm'] )) {
			...
			jQuery( '#customize-header-actions .spinner' )
				.after( '<button class="button customize-action-refresh icon-spin3" title="' + kicker_customizer_vars['msg_reload_title'] + '">' + kicker_customizer_vars['msg_reload'] + '</button>' );
			...
			if (kicker_customizer_vars && kicker_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( kicker_customizer_vars['theme_name_suffix'] );
			}
			...
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' )
				.on( 'change', kicker_customizer_update_load_fonts );
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' )
				.on( 'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							kicker_customizer_check_dependencies( section );
						}
					}
				} );

			// Decorate subsections in the panel 'Typography'
			if ( kicker_customizer_vars['decorate_fonts'] ) {

				jQuery('body').addClass( 'kicker_decorate_fonts_section' );

				// Prepare section for open on title is clicked
				var kicker_customizer_title_clicked = function( title ) {
					var id = title.parent().attr( 'aria-owns' );
					kicker_customizer_close_opened_sections( title.parents( '#sub-accordion-panel-fonts' ), id );
					var section = jQuery( '#' + id ),
					...
					.each ( function() {
						...
						$self
							.on( 'keydown', function(e) {
								if ( e.which == 13 ) {
									kicker_customizer_title_clicked( $self );
								}
							} )
							.on( 'click', function() {
								kicker_customizer_title_clicked( $self );
				...
				var kicker_customizer_close_opened_sections = function( panel, current_id ) {
					if ( panel.length ) {
				...
				jQuery( '#customize-theme-controls #sub-accordion-panel-fonts .customize-panel-back' )
					.on( 'keydown', function(e) {
						if ( e.which == 13 ) {
							kicker_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
						}
					} )
					.on( 'click', function() {
						kicker_customizer_close_opened_sections( jQuery(this).parents('#sub-accordion-panel-fonts') );
	...
	api.bind( 'change', function(obj) {
		...
		kicker_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
		kicker_customizer_refresh_preview( obj );
	} );
	...
	api.bind( 'refresh_schemes', function() {
		...
		for (i in cssTemplate) {
			...
			i = i.replace( 'scheme_', '' );
			if (typeof kicker_color_schemes[i] === 'undefined') {
				delete cssTemplate['scheme_' + i];
				jQuery( '#tmpl-kicker-color-scheme-' + i ).remove();
			} else if (tpl_idx == '') {
				tpl_idx     = i;
				tpl_content = jQuery( '#tmpl-kicker-color-scheme-' + i ).html().trim();
			}
		}
		// Add new templates
		var regex = new RegExp( "\.scheme_" + tpl_idx, "g" );
		for (i in kicker_color_schemes) {
			if (typeof cssTemplate['scheme_' + i] === 'undefined') {
				jQuery( '#tmpl-kicker-color-scheme-' + tpl_idx )
				.clone()
				.attr( 'id', 'tmpl-kicker-color-scheme-' + i )
				.html( tpl_content.replace( regex, '.scheme_' + i ) )
				.insertAfter( jQuery( '#tmpl-kicker-color-scheme-' + tpl_idx ) );
				cssTemplate['scheme_' + i] = wp.template( 'kicker-color-scheme-' + i );
			}
		}
	} );
	...
	for (var action in kicker_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (kicker_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = kicker_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = kicker_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function kicker_customizer_get_field_value(fld) {
	...
	function kicker_customizer_check_dependencies(cont) {
			...
			function() {
				...
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = kicker_customizer_get_field_value( fld );
							if ( val == 'inherit' ) {
								dep_cnt = 0;
								dep_all = 1;
								var tag;
	...
	function kicker_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( kicker_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					kicker_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in kicker_theme_fonts) {
					for (rule in kicker_theme_fonts[opt]) {
						if (opt + '_' + rule == id) {
							// Store new value to the fonts table
							kicker_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
	...
	// Actions buttons
	function kicker_customizer_actions(e) {
	...
	function kicker_customizer_update_theme_vars(opt, value) {
		kicker_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function kicker_customizer_update_theme_fonts(opt, rule, value) {
		kicker_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function kicker_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= kicker_customizer_vars['max_load_fonts']; i++) {
	...
	function kicker_customizer_update_css() {
		...
		var vars = kicker_customizer_add_theme_vars( kicker_theme_vars );

		// Make styles and add its to the css
		css += kicker_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = kicker_customizer_add_theme_fonts( kicker_theme_fonts );

		// Make styles and add its to the css
		css += kicker_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in kicker_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( kicker_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in kicker_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in kicker_color_schemes[scheme].colors) {
				colors[i] = kicker_color_schemes[scheme].colors[i];
			}
			// Add theme specific colors and tints
			colors = kicker_customizer_add_theme_colors( colors );
			...
			var tmpl = jQuery( '#tmpl-kicker-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function kicker_customizer_add_theme_colors(colors) {
		if (kicker_additional_colors) {
			var clr = '', v = '';
			for (var k in kicker_additional_colors) {
				v   = kicker_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag + '_font-size']       = typeof fonts[tag]['font-size'] != 'undefined'
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + kicker_customizer_prepare_css_value( fonts[tag]['font-size'] ) + ";"
												: '';
			...
			rez[tag + '_margin-top']      = typeof fonts[tag]['margin-top'] != 'undefined'
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + kicker_customizer_prepare_css_value( fonts[tag]['margin-top'] ) + ";"
												: '';
			rez[tag + '_margin-bottom']   = typeof fonts[tag]['margin-bottom'] != 'undefined'
									&& fonts[tag]['margin-bottom'] !== ''
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + kicker_customizer_prepare_css_value( fonts[tag]['margin-bottom'] ) + ";"
												: '';
		}
		return rez;
	}
	...
	function kicker_customizer_add_theme_vars(vars) {
		...
		if ( typeof vars['rad'] != 'undefined' ) {
			...
			rez['rad']      = kicker_customizer_prepare_css_value( vars['rad'] );
		...
		if ( typeof vars['page_width'] != 'undefined' ) {
			...
			if ( isNaN( vars['page_width'] ) || vars['page_width'] === 0) {
				vars['page_width'] = kicker_customizer_vars['page_width_default'];
			}
			vars['sidebar_width']      = parseInt( vars['sidebar_width'], 10 );
			vars['sidebar_gap']        = parseInt( vars['sidebar_gap'], 10 );

			rez['page_width']          = kicker_customizer_prepare_css_value( vars['page_width'] );
			rez['page_boxed_extra']    = kicker_customizer_prepare_css_value( vars['page_boxed_extra'] );
			rez['page_fullwide_extra'] = kicker_customizer_prepare_css_value( vars['page_fullwide_extra'] );
			rez['page_fullwide_max']   = kicker_customizer_prepare_css_value( vars['page_fullwide_max'] );
			rez['grid_gap']            = kicker_customizer_prepare_css_value( vars['grid_gap'] );
	...
	// Add ed to css value
	function kicker_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).substr( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function kicker_customizer_prepare_html_value(val) {

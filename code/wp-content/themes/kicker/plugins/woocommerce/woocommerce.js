...
jQuery( document ).on( 'action.ready_kicker', function() {
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.kicker_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		kicker_set_cookie( 'kicker_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// On first run
	kicker_woocommerce_add_inc_dec();
	...
	// Add buttons inc/dec
	function kicker_woocommerce_add_inc_dec() {
		jQuery( '.woocommerce div.quantity:not(.inited),.woocommerce-page div.quantity:not(.inited)' ).each( function() {
			...
			$self
				...
				.on( 'click', '>span', function(e) {
					kicker_woocomerce_inc_dec_click( jQuery( this ) );
					e.preventDefault();
					return false;
				} );
		} );
	...
	function kicker_woocomerce_inc_dec_click(button) {
	...
	$document.on( 'action.scroll_kicker', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < kicker_window_scroll_top() + kicker_window_height() * 1.5 ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		kicker_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					kicker_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					kicker_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		kicker_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			kicker_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function kicker_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = kicker_window_width() + ( kicker_window_height() < $document.height() || kicker_window_scroll_top() > 0 ? 16 : 0 );
				}
				...
				if ( w >= KICKER_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {

...
if ( ! defined( 'KICKER_FSE_TEMPLATE_PART_PT' ) ) define( 'KICKER_FSE_TEMPLATE_PART_PT', 'wp_template_part' );

// Load additional files with FSE support
require_once kicker_get_file_dir( 'plugins/gutenberg/gutenberg-fse-lists.php' );
require_once kicker_get_file_dir( 'plugins/gutenberg/gutenberg-fse-templates.php' );


if ( ! function_exists( 'kicker_gutenberg_fse_loader' ) ) {
	add_action( 'wp_loaded', 'kicker_gutenberg_fse_loader', 1 );
	/**
	 * Turn off a Gutenberg (plugin) templates on frontend
	 * if a theme-specific frontend editor is active.
	 */
	function kicker_gutenberg_fse_loader() {
		// 
		if ( kicker_exists_gutenberg_fse()
			&& get_option( 'show_on_front' ) == 'page'
			&& (int) get_option('page_on_front') > 0
			&& kicker_is_on( kicker_get_theme_option( 'front_page_enabled', false ) )
			&& kicker_get_current_url() == '/'
		) {
...
if ( ! function_exists( 'kicker_gutenberg_is_fse_wp_support' ) ) {
	/**
	 * Check if WordPress with FSE support installed
	 * (WordPress 5.9+ Site Editor is present).
	 */
	function kicker_gutenberg_is_fse_wp_support() {
		return function_exists( 'wp_is_block_theme' );
	}
}

if ( ! function_exists( 'kicker_gutenberg_is_fse_gb_support' ) ) {
	/**
	 * Check if a plugin "Gutenberg" with FSE support is installed.
	 */
	function kicker_gutenberg_is_fse_gb_support() {
		return function_exists( 'gutenberg_is_fse_theme' );
	}
}

if ( ! function_exists( 'kicker_exists_gutenberg_fse' ) ) {
	/**
	 * Check if FSE is available
	 * (WordPress 5.9+ or the plugin "Gutenberg" are installed).
	 */
	function kicker_exists_gutenberg_fse() {
		return kicker_exists_gutenberg() && ( kicker_gutenberg_is_fse_wp_support() || kicker_gutenberg_is_fse_gb_support() );
	}
}

if ( ! function_exists( 'kicker_gutenberg_is_fse_theme' ) ) {
	/**
	 * Check if the current theme supports a FSE behaviour.
	 */
	function kicker_gutenberg_is_fse_theme() {
		static $fse = -1;
		if ( $fse == -1 ) {
			if ( kicker_gutenberg_is_fse_wp_support() ) {                                    // WordPress 5.9+ FSE
				$fse = (bool) wp_is_block_theme();
			} else if ( kicker_gutenberg_is_fse_gb_support() ) {                             // Plugin Gutenberg FSE
				$fse = (bool) gutenberg_is_fse_theme();
			} else {
				$fse = is_readable( KICKER_THEME_DIR . 'templates/index.html' )              // WordPress 5.9+ FSE
						|| ( KICKER_THEME_DIR != KICKER_CHILD_DIR && is_readable( KICKER_CHILD_DIR . 'templates/index.html' ) )
						|| is_readable( KICKER_THEME_DIR . 'block-templates/index.html' )    // Plugin Gutenberg FSE
						|| ( KICKER_THEME_DIR != KICKER_CHILD_DIR && is_readable( KICKER_CHILD_DIR . 'block-templates/index.html' ) );
			}
		}
		return $fse;
	}
...
if ( ! function_exists( 'kicker_gutenberg_is_fse_enabled' ) ) {
	/**
	 * Check if the current theme supports FSE behaviour and FSE is installed and activated
	 */
	function kicker_gutenberg_is_fse_enabled() {
		return kicker_exists_gutenberg_fse() && kicker_gutenberg_is_fse_theme();
	}
}

if ( ! function_exists( 'kicker_gutenberg_fse_copy_from_skin' ) ) {
	add_action( 'kicker_action_skin_switched', 'kicker_gutenberg_fse_copy_from_skin', 10, 2 );
	/**
	 * Copy FSE-required folders from a new skin subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'kicker_action_skin_switched', 'kicker_gutenberg_fse_copy_from_skin', 10, 2 );
	 */
	function kicker_gutenberg_fse_copy_from_skin( $new_skin, $old_skin ) {
		$theme_templates_dir = kicker_prepare_path( KICKER_THEME_DIR . ( kicker_gutenberg_is_fse_wp_support() ? 'templates/' : 'block-templates/' ) );
		$theme_template_parts_dir = kicker_prepare_path( KICKER_THEME_DIR . ( kicker_gutenberg_is_fse_wp_support() ? 'parts/' : 'block-template-parts/' ) );
		$theme_json = kicker_prepare_path( KICKER_THEME_DIR . 'theme.json' );
		$skin_templates_dir = kicker_prepare_path( KICKER_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( kicker_gutenberg_is_fse_wp_support() ? '/templates/' : '/block-templates/' ) );
		$skin_template_parts_dir = kicker_prepare_path( KICKER_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( kicker_gutenberg_is_fse_wp_support() ? '/parts/' : '/block-template-parts/' ) );
		// Remove old templates from the stylesheet dir (if exists)
		if ( is_dir( $theme_templates_dir ) ) {
			kicker_unlink( $theme_templates_dir );
		}
		if ( is_dir( $theme_template_parts_dir ) ) {
			kicker_unlink( $theme_template_parts_dir );
		}
		if ( file_exists( $theme_json ) ) {
			kicker_unlink( $theme_json );
		}
		...
		if ( is_dir( $skin_templates_dir ) ) {
			kicker_copy( $skin_templates_dir, $theme_templates_dir );
		}
		if ( is_dir( $skin_template_parts_dir ) ) {
			kicker_copy( $skin_template_parts_dir, $theme_template_parts_dir );
		}
	}
}

if ( ! function_exists( 'kicker_gutenberg_fse_update_theme_json' ) ) {
	add_action( 'kicker_action_skin_switched', 'kicker_gutenberg_fse_update_theme_json', 30, 2 );
	add_action( 'kicker_action_save_options', 'kicker_gutenberg_fse_update_theme_json', 30 );
	add_action( 'trx_addons_action_save_options', 'kicker_gutenberg_fse_update_theme_json', 30 );
	/**
	 * Update the file 'theme.json' after the current skin is switched and/or theme options are saved.
	 * 
	 * Hooks: add_action( 'kicker_action_skin_switched', 'kicker_gutenberg_fse_update_theme_json', 30, 2 );
	 *        add_action( 'kicker_action_save_options', 'kicker_gutenberg_fse_update_theme_json', 30 );
	 *        add_action( 'trx_addons_action_save_options', 'kicker_gutenberg_fse_update_theme_json', 30 );
	 * 
	 * Trigger filter 'kicker_filter_gutenberg_fse_theme_json_data' to allow other modules modify this data before saving.
	 */
	function kicker_gutenberg_fse_update_theme_json( $new_skin = '', $old_skin = '' ) {
		// If the current skin supports FSE
		if ( ! empty( $new_skin ) && is_dir( KICKER_THEME_DIR . kicker_skins_get_current_skin_dir( $new_skin ) . '_fse' ) ) {
			$theme_json_data = kicker_gutenberg_fse_theme_json_data();
			if ( ! empty( $theme_json_data ) && is_array( $theme_json_data ) ) {
				kicker_fpc( kicker_prepare_path( KICKER_THEME_DIR . 'theme.json' ), json_encode( $theme_json_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) );
			}
		}
	}
}

if ( ! function_exists( 'kicker_gutenberg_fse_theme_json_data' ) ) {
	/**
	 * Return a default data to save to the file 'theme.json'
	 * 
	 * Trigger filter 'kicker_filter_gutenberg_fse_theme_json_data' to allow other modules change data.
	 * 
	 * @return array  Data for the 'theme.json'
	 */
	function kicker_gutenberg_fse_theme_json_data() {
		...
		$data = array(
					...
					"customTemplates" => array(
						array(
							"name"      => "blog",
							"title"     => esc_html__( "Blog archive", 'kicker' ),
							"postTypes" => array(
								"page"
							)
						)
					...
					"templateParts" => array(
						array(
							"name"  => "header",
							"title" => esc_html__( "Header for FSE", 'kicker' ),
							"area"  => "header",
						),
						array(
							"name"  => "sidebar",
							"title" => esc_html__( "Sidebar for FSE", 'kicker' ),
							"area"  => "uncategorized",
						),
						array(
							"name"  => "content-404",
							"title" => esc_html__( "Page 404 content", 'kicker' ),
							"area"  => "uncategorized",
						),
						array(
							"name"  => "blog-post-standard",
							"title" => esc_html__( "Blog post item: Standard", 'kicker' ),
							"area"  => "uncategorized",
						),
						array(
							"name"  => "blog-post-header",
							"title" => esc_html__( "Blog post header", 'kicker' ),
							"area"  => "uncategorized",
						),
						array(
							"name"  => "blog-pagination",
							"title" => esc_html__( "Blog pagination", 'kicker' ),
							"area"  => "uncategorized",
						),
						array(
							"name"  => "footer",
							"title" => esc_html__( "Footer for FSE", 'kicker' ),
							"area"  => "footer",
						),
					),
...
*/
		...
		$data['settings']['color']['duotone'][] = array(
			...
			"slug"   => 'bg-and-text',
			"name"   => esc_html__( 'Background and text color', 'kicker' )
		);
		$data['settings']['color']['duotone'][] = array(
			"colors" => array( $scheme['bg_color'], $scheme['text_dark'] ),
			"slug"   => 'bg-and-dark',
			"name"   => esc_html__( 'Background and dark color', 'kicker' )
		);
		$data['settings']['color']['duotone'][] = array(
			"colors" => array( $scheme['bg_color'], $scheme['text_link'] ),
			"slug"   => 'bg-and-link',
			"name"   => esc_html__( 'Background and link color', 'kicker' )
		);
		...
		$data['settings']['color']['gradients'][] = array(
			...
			"name"     => esc_html__( 'Vertical from link color to hover color', 'kicker' ),
		);
		$data['settings']['color']['gradients'][] = array(
			...
			"name"     => esc_html__( 'Diagonal from link color to hover color', 'kicker' ),
		);
/*
		// Add fonts
...
*/
		...
		return apply_filters( 'kicker_filter_gutenberg_fse_theme_json_data', $data );
	}
}

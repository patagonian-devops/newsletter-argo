	...
	foreach( $template_parts as $k=>$v ) {
		...
		// Post title
			...
			echo '<'.esc_html($post_title_tag).' class="sc_blogger_item_title entry-title'.($args['post_title_tag'] == 'h1' ? ' h1' : '').'"'
						. ' data-item-number="' . esc_attr(empty($args['numbers']) ? kicker_skin_num_to_roman($args['item_number']) : $args['item_number'] . ($args['template_'.$args['type']] == 'simple' ? '.' : '')) . '"'
						. '>'
							. (!empty($post_link)

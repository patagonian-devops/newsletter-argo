...
if (!trx_addons_is_off($args['pagination']) && $max_page > 1) {
	...
	if ($args['pagination'] == 'prev_next') {
		?><nav class="<?php echo esc_attr($sc); ?>_pagination sc_item_pagination sc_item_pagination_<?php echo esc_attr($args['pagination']); ?> nav-links-old <?php echo esc_attr($align); ?>" data-params="<?php echo esc_attr(serialize(apply_filters('trx_addons_filter_sc_args_to_serialize', $args))); ?>" role="navigation"><?php
			?><span class="nav-prev<?php if ($args['page'] == 1) echo ' nav-disabled'; ?>"><a href="#" data-page="<?php echo esc_attr($args['page'] - 1); ?>"><span class="nav-prev-label"><?php esc_html_e('Previous', 'kicker'); ?></span></a></span><?php
			?><span class="nav-next<?php if ($args['page'] >= $max_page) echo ' nav-disabled'; ?>"><a href="#" data-page="<?php echo esc_attr($args['page'] + 1); ?>"><span class="nav-next-label"><?php esc_html_e('Next', 'kicker'); ?></span></a></span><?php
		?></nav><?php
	...
	// Page numbers
		...
		?><nav class="<?php echo esc_attr($sc); ?>_pagination sc_item_pagination sc_item_pagination_<?php echo esc_attr($args['pagination']); ?> navigation pagination <?php echo esc_attr($align); ?>" data-params="<?php echo esc_attr(serialize(apply_filters('trx_addons_filter_sc_args_to_serialize', $args))); ?>" role="navigation">
			<div class="nav-links"><?php
				...
				if ($args['page'] > 1) {
					?><a href="#" class="page-numbers first" data-page="1" title="<?php echo esc_attr('First page', 'kicker'); ?>"><?php esc_html_e('First', 'kicker'); ?></a><?php
					?><a href="#" class="page-numbers prev" data-page="<?php echo esc_attr($args['page'] - 1); ?>" title="<?php echo esc_attr('Previous page', 'kicker'); ?>"><?php esc_html_e('Previous', 'kicker'); ?></a><?php
				}
				for ($i = $start; $i <= $end; $i++) {
					if ($i == $args['page']) {
				...
				if ($args['page'] < $max_page) {
					?><a href="#" class="page-numbers next" data-page="<?php echo esc_attr($args['page'] + 1); ?>" title="<?php echo esc_attr('Next page', 'kicker'); ?>"><?php esc_html_e('Next', 'kicker'); ?></a><?php
					?><a href="#" class="page-numbers last" data-page="<?php echo esc_attr($max_page); ?>" title="<?php echo esc_attr('Last page', 'kicker'); ?>"><?php esc_html_e('Last', 'kicker'); ?></a><?php
				}
			?></div><?php
			if ( $args['pagination'] == 'advanced_pages' ) {
				?><span class="page-numbers page-count"><?php echo sprintf(esc_html__('Page %d of %d', 'kicker'), $args['page'], $max_page); ?></span><?php
			}
		?></nav><?php
		...
		if ($args['page'] < $max_page) {
			...
			?>" data-params="<?php echo esc_attr(serialize(apply_filters('trx_addons_filter_sc_args_to_serialize', $args))); ?>">
				<a class="nav-links <?php echo esc_attr($pagination_btn); ?>" data-page="<?php echo esc_attr($args['page']+1); ?>" data-max-page="<?php echo esc_attr($max_page); ?>"><span><?php
					echo !empty( $args['more_text'] ) ? esc_html( $args['more_text'] ) : ( array_key_exists('pagination_style', $args) && $args['pagination_style'] == 'rounded' ? esc_html__( 'More', 'kicker' ) : esc_html__( 'Load more', 'kicker' ));
				?></span></a>
			</nav><?php
		}
	}

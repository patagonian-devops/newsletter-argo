...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'kicker_trx_addons_theme_specific_components' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'kicker_trx_addons_theme_specific_components' );
	function kicker_trx_addons_theme_specific_components( $enable = false ) {
		return KICKER_THEME_FREE
					? false     // Free version
					: false;     // Pro version or Developer mode
	}
}

if ( ! function_exists( 'kicker_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'kicker_trx_addons_theme_specific_setup1', 1 );
	function kicker_trx_addons_theme_specific_setup1() {
		if ( kicker_exists_trx_addons() ) {
			add_filter( 'trx_addons_addons_list', 'kicker_trx_addons_addons_list', 100 );
			add_filter( 'trx_addons_api_list', 'kicker_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'kicker_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'kicker_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'kicker_trx_addons_widgets_list' );
		}
	}
}

// Addons
if ( ! function_exists( 'kicker_trx_addons_addons_list' ) ) {
	//Handler of the add_filter( 'trx_addons_addons_list', 'kicker_trx_addons_addons_list', 100 );
	function kicker_trx_addons_addons_list( $list = array() ) {
...
// API
if ( ! function_exists( 'kicker_trx_addons_api_list' ) ) {
	//Handler of the add_filter('trx_addons_api_list',	'kicker_trx_addons_api_list');
	function kicker_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'kicker_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'kicker_trx_addons_cpt_list');
	function kicker_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'kicker_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'kicker_trx_addons_sc_list');
	function kicker_trx_addons_sc_list( $list = array() ) {
...
// Widgets
if ( ! function_exists( 'kicker_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'kicker_trx_addons_widgets_list');
	function kicker_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'kicker_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'kicker_trx_addons_menu_cache' );
	function kicker_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'kicker_trx_addons_localize_script' ) ) {
	...
	function kicker_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = kicker_get_scheme_color( 'alter_link' );
		return $arr;
	}
}

// Add theme-specific width where used min 2 columns
if ( ! function_exists( 'kicker_trx_addons_max_one_column_width' ) ) {
	add_filter( 'trx_addons_filter_max_one_column_width', 'kicker_trx_addons_max_one_column_width' );
	function kicker_trx_addons_max_one_column_width( $w ) {
		$media = kicker_storage_get_array( 'responsive', 'sm_wp' );
		if ( empty( $media['max'] ) ) {
			$media = array( 'max' => 600 );
		}
		return $media['max'];
...
if ( ! function_exists( 'kicker_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'kicker_trx_addons_sc_type', 10, 2 );
	function kicker_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = kicker_array_merge( $list, kicker_get_list_blog_styles( false, 'sc' ) );
		}

		if ( 'trx_sc_layouts_iconed_text' == $sc ) {
			$list['menu'] = esc_html__('Menu', 'kicker');
		}
		return $list;	
	}
}
...
if ( ! function_exists( 'kicker_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'kicker_trx_addons_sc_prepare_atts', 10, 2 );
	function kicker_trx_addons_sc_prepare_atts( $atts, $sc ) {
		// Blogger
		if ( 'trx_sc_blogger' == $sc ) {
			$list = kicker_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				...
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = kicker_get_custom_blog_id( $atts['type'] );
					...
					$use_masonry = kicker_is_blog_style_use_masonry( $atts['type'] );
				}
...
if ( ! function_exists( 'kicker_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'kicker_trx_addons_sc_atts', 10, 2 );
	function kicker_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'kicker_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'kicker_trx_addons_sc_output', 10, 4 );
	function kicker_trx_addons_sc_output( $output, $sc, $atts, $content ) {
		$sc = str_replace( array( 'trx_widget', 'trx_' ), array( 'sc_widget', '' ), $sc );
		if ( substr( $sc, -3 ) == 'map' ) {
			$sc = str_replace( 'map', 'map_content', $sc );
		}
		if ( ! empty( $atts['scheme'] ) && ! kicker_is_inherit( $atts['scheme'] ) ) {
			$output = str_replace( 'class="' . esc_attr( $sc ) . ' ', 'class="' . esc_attr( $sc ) . ' scheme_' . esc_attr( $atts['scheme'] ) . ' ', $output );
		}

		// Button
		if ( 'sc_button' == $sc ) { 
			if ( ! empty( $atts['color_style'] ) && ! kicker_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'kicker_trx_addons_sc_item_link_classes' ) ) {
	add_filter( 'trx_addons_filter_sc_item_link_classes', 'kicker_trx_addons_sc_item_link_classes', 10, 3 );
	function kicker_trx_addons_sc_item_link_classes( $class, $sc, $atts=array() ) {
		if ( 'sc_button' == $sc ) {
			if ( ! empty( $atts['color_style'] ) && ! kicker_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
				$class .= ' color_style_' . esc_attr( $atts['color_style'] );
			}
			if ( ! empty( $atts['hover_style'] ) && ! kicker_is_inherit( $atts['hover_style'] ) && 'default' != $atts['hover_style'] ) {
...
if ( ! function_exists( 'kicker_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'kicker_trx_addons_sc_item_title_tag' );
	function kicker_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Add new styles to the Google map
if ( ! function_exists( 'kicker_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'kicker_trx_addons_sc_googlemap_styles' );
	function kicker_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'kicker' );
		return $list;
	}
}

// Show post info from CPT Portfolio instead post meta
if ( ! function_exists( 'kicker_trx_addons_portfolio_info' ) ) {
	add_filter( 'kicker_filter_show_blog_meta', 'kicker_trx_addons_portfolio_info', 10, 2 );
	function kicker_trx_addons_portfolio_info( $show, $meta_parts ) {
		if ( kicker_exists_trx_addons() && defined( 'TRX_ADDONS_CPT_PORTFOLIO_PT' ) && get_post_type() == TRX_ADDONS_CPT_PORTFOLIO_PT && function_exists( 'trx_addons_cpt_portfolio_show_details' ) ) {
...
if ( ! function_exists( 'kicker_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'kicker_trx_addons_tiny_mce_style_formats' );
	function kicker_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'kicker' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'kicker' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'kicker_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'kicker_filter_allow_override_header_image', 'kicker_trx_addons_allow_override_header_image' );
	function kicker_trx_addons_allow_override_header_image( $allow ) {
		return is_single()
				&& (
					kicker_is_team_page()
					|| kicker_is_cars_page()
...
if ( ! function_exists( 'kicker_trx_addons_meta_box_fields' ) ) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'kicker_trx_addons_meta_box_fields', 10, 2 );
	function kicker_trx_addons_meta_box_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'kicker' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'kicker' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
...
if ( ! function_exists( 'kicker_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'kicker_trx_addons_thumb_size', 10, 2 );
	function kicker_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
		return $thumb_size;
	}
}
...
if ( ! function_exists( 'kicker_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'kicker_trx_addons_slider_title', 10, 3 );
	function kicker_trx_addons_slider_title( $title, $data, $args ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'kicker_skin_trx_addons_frontend_scripts' ) ) {
	add_action( 'wp_enqueue_scripts', 'kicker_skin_trx_addons_frontend_scripts', 1100 );
	function kicker_skin_trx_addons_frontend_scripts() {
		if ( kicker_exists_trx_addons() ) {	
			if ( trx_addons_reviews_enable() && is_singular('post') ) {			
				wp_enqueue_style( 'kicker-trx-addons-reviews', kicker_get_file_url( kicker_skins_get_current_skin_dir() . 'plugins/trx_addons/trx_addons-reviews.css' ), array(), null );
			}

			if ( kicker_is_on( kicker_get_theme_option( 'debug_mode' ) ) ) {
				$kicker_url = kicker_get_file_url( kicker_skins_get_current_skin_dir() . 'plugins/trx_addons/trx_addons.js' );
				if ( '' != $kicker_url ) {
					wp_enqueue_script( 'kicker-trx-addons', $kicker_url, array( 'jquery', 'kicker-utils', 'kicker-init' ), null, true );
				}
			}
...
if ( ! function_exists( 'kicker_skin_trx_addons_merge_scripts' ) ) {
	add_filter( 'kicker_filter_merge_scripts', 'kicker_skin_trx_addons_merge_scripts' );
	function kicker_skin_trx_addons_merge_scripts( $list ) {
		$list[] = kicker_skins_get_current_skin_dir() . 'plugins/trx_addons/trx_addons.js';
		return $list;
	}
}

// Widgets arguments
if ( ! function_exists( 'kicker_trx_addons_widget_args' ) ) {
	add_filter( 'trx_addons_filter_widget_args', 'kicker_trx_addons_widget_args', 10, 3 );
	function kicker_trx_addons_widget_args($array, $instance, $sc) {
		// Recent News
		...
		if ( $sc == 'trx_addons_widget_aboutme') {
			...
			if ( 'default' == $instance['type'] ) {
				$avatar = kicker_get_attachment_url( $avatar, kicker_get_thumb_size('med-square') );
			}
			if ( 'modern' == $instance['type'] ) {
				$avatar = kicker_get_attachment_url( $avatar, kicker_get_thumb_size('about-us-modern') );
			}
			if (!empty($avatar)) {				
				if (!empty($avatar)) {
...
if ( ! function_exists( 'kicker_trx_addons_widget_args_update' ) ) {
	add_filter( 'trx_addons_filter_widget_args_update', 'kicker_trx_addons_widget_args_update', 10, 3 );
	function kicker_trx_addons_widget_args_update($instance, $new_instance, $sc) {
		// Recent News		
		if ( $sc == 'trx_addons_widget_recent_news') {
...
if ( ! function_exists( 'kicker_trx_addons_widget_args_default' ) ) {
	add_filter( 'trx_addons_filter_widget_args_default', 'kicker_trx_addons_widget_args_default', 10, 2 );
	function kicker_trx_addons_widget_args_default($array, $sc) {		
		// Recent News
		if ( $sc == 'trx_addons_widget_recent_news') {
...
if ( ! function_exists( 'kicker_trx_addons_after_widget_title' ) ) {
	add_action( 'trx_addons_action_after_widget_title', 'kicker_trx_addons_after_widget_title', 10, 3 );
	function kicker_trx_addons_after_widget_title($instance, $sc, $widget) {		
		// Recent Posts
		if ( $sc == 'trx_addons_widget_recent_posts') {
			$widget->show_field( 
								array(
									'name' => 'type',
									'title' => esc_html__('Widget type:', 'kicker'),
									'value' => $instance['type'],
									'options' => array(
													'default' => esc_html__('Default', 'kicker'),
													'classic' => esc_html__('Classic', 'kicker'),
													'modern' => esc_html__('Modern', 'kicker')
													),
									'type' => 'select'
									)
								);	
		...
		if ( $sc == 'trx_addons_widget_video') {
			$widget->show_field( 
								array(
									'name' => 'button_style',
									'title' => esc_html__('Button style:', 'kicker'),
									'value' => array_key_exists('button_style', $instance) ? $instance['button_style'] : '',
									'options' => array(
													'default' => esc_html__('Default', 'kicker'),
													'bordered' => esc_html__('Bordered', 'kicker')
													),
									'type' => 'select'
									)
								);
		...
		if ( $sc == 'trx_addons_widget_video_list') {
			$widget->show_field( 
								array(
									'name' => 'type',
									'title' => esc_html__('Type:', 'kicker'),
									'value' => $instance['type'],
									'options' => array(
													'default' => esc_html__('Default', 'kicker'),
													'alter' => esc_html__('Alter', 'kicker'),
													'wide' => esc_html__('Wide', 'kicker'),
													'news' => esc_html__('News', 'kicker'),
													'classic' => esc_html__('Classic', 'kicker')
													),
									'type' => 'select'
									)
			...
			$widget->show_field( 
								array(
									'name' => 'content_style',
									'title' => esc_html__('Content style:', 'kicker'),
...
if ( ! function_exists( 'kicker_trx_addons_before_widget_filters' ) ) {
	add_action( 'trx_addons_action_before_widget_filters', 'kicker_trx_addons_before_widget_filters', 10, 3 );
	function kicker_trx_addons_before_widget_filters($instance, $sc, $widget) {	
		// Blogger
		if ( $sc == 'trx_addons_sow_widget_blogger') {
			$widget->show_field( 
								array(
									'name' => 'pagination_align',
									'title' => esc_html__('Pagination align:', 'kicker'),
			...
			$widget->show_field( 
								array(
									...
									'title' => esc_html__('Pagination color:', 'kicker'),
									'value' => $instance['pagination_color'],
									'options' => kicker_skin_get_list_sc_color_styles(),
									'dependency' => array(
										'pagination' => array( 'load_more' ),
									),
									'type' => 'select'
			...
			$widget->show_field( 
								array(
									...
									'title' => esc_html__('Pagination hover:', 'kicker'),
									'value' => $instance['pagination_hover'],
									'options' => kicker_skin_get_list_sc_hover_styles(false),
									'dependency' => array(
										'pagination' => array( 'load_more' ),
									),
									'type' => 'select'
...
if ( ! function_exists( 'kicker_trx_addons_before_widget_meta' ) ) {
	add_action( 'trx_addons_action_before_widget_meta', 'kicker_trx_addons_before_widget_meta', 10, 3 );
	function kicker_trx_addons_before_widget_meta($instance, $sc, $widget) {				
		// Blogger
		if ( $sc == 'trx_addons_sow_widget_blogger') {
			$widget->show_field( 
								array(
									...
									'title' => '',
									'label' => esc_html__('Full size image', 'kicker'),
			...
			$widget->show_field( 
								array(
									...
									'title' => esc_html__('Info over image:', 'kicker'),
									'description' => esc_html__('Show info over image for the post format Image.', 'kicker'),
									...
									'options' => array(
													...
													'cc' => esc_html__('Center center', 'kicker'),
													'bc' => esc_html__('Bottom center', 'kicker'),
													'bl' => esc_html__('Bottom left', 'kicker'),
													'bl_2' => esc_html__('Bottom left and category on the top', 'kicker'),
													),
									'type' => 'select'
									)
								);
			$widget->show_field( 
								array(
									'name' => 'post_title_tag',
									'title' => esc_html__('Titles tag:', 'kicker'),
									'value' => $instance['post_title_tag'],
									'dependency' => array(
										'type' => array( 'default', 'wide', 'list', 'news' ),
									),
									'options' => array(
													'h1' => esc_html__('Heading 1', 'kicker'),
													'h2' => esc_html__('Heading 2', 'kicker'),
													'h3' => esc_html__('Heading 3', 'kicker'),
													'h4' => esc_html__('Heading 4', 'kicker'),
													'h5' => esc_html__('Heading 5', 'kicker'),
													'h6' => esc_html__('Heading 6', 'kicker'),
													),
									'type' => 'select'
									)
								);
			$widget->show_field( 
								array(
									...
									'title' => esc_html__('Titles tag (over image):', 'kicker'),
			...
			$widget->show_field( 
								array(
									...
									'title' => esc_html__('Title length (in letters):', 'kicker'),
			...
			$widget->show_field( 
								array(
									'name' => 'column_gap',
									'title' => esc_html__('Gap between posts:', 'kicker'),
									...
									'options' => array(
													'no' => esc_html__('No gap', 'kicker'),
													'small' => esc_html__('Small gap', 'kicker'),
													'normal' => esc_html__('Normal gap', 'kicker'),
									),
									'type' => 'select'
									)
								);
...
if ( ! function_exists( 'kicker_trx_addons_after_widget_fields' ) ) {
	add_action( 'trx_addons_action_after_widget_fields', 'kicker_trx_addons_after_widget_fields', 10, 3 );
	function kicker_trx_addons_after_widget_fields($instance, $sc, $widget) {		
		// Recent News
		if ( $sc == 'trx_addons_widget_recent_news') {
			$widget->show_field( 
								array(
									'name' => 'magazine_type',
									'title' => esc_html__('Magazine type:', 'kicker'),
									...
									'options' => array(
													'default' => esc_html__('Default', 'kicker'),
													'on_plate' => esc_html__('On Plate', 'kicker'),
													'with_rating' => esc_html__('With Rating', 'kicker')
													),
									'type' => 'select'
									)
								);
			...
			$widget->show_field( 
								array(
									'name' => 'button_link',
									'title' => esc_html__('Button link:', 'kicker'),
			...
			$widget->show_field( 
                                array(
                                    'name' => 'button_text',
                                    'title' => esc_html__('Button text:', 'kicker'),
...
if ( ! function_exists( 'kicker_trx_addons_sc_recent_news_template_args' ) ) {
	add_filter( 'trx_addons_filter_sc_recent_news_template_args', 'kicker_trx_addons_sc_recent_news_template_args', 10, 2 );
	function kicker_trx_addons_sc_recent_news_template_args($array, $atts) {		
...
if ( ! function_exists( 'kicker_elm_add_new_controls' ) ) {
	add_action( 'elementor/element/before_section_end', 'kicker_elm_add_new_controls', 10, 3 );
	function kicker_elm_add_new_controls( $element, $section_id, $args ) {		
		if ( is_object( $element ) ) {
			...
			if ( 'trx_sc_recent_news' == $el_name  && 'section_sc_recent_news' === $section_id ) {
				$element->add_control(
					'magazine_type', array(
						'label'       => esc_html__( 'Magazine type', 'kicker' ),
						'label_block' => false,
				...
				$element->add_control(
					'button_link', array(
						'label'       => esc_html__( 'Button link', 'kicker' ),
						'label_block' => false,
				...
				$element->add_control(
					'button_text', array(
						'label'       => esc_html__( 'Button text', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::TEXT,
						'condition' => [
								'style' => ['news-magazine']
			...
			if ( 'trx_sc_blogger' == $el_name  && 'section_sc_blogger' === $section_id ) {
				$element->add_control(
					'pagination_align', array(
						'label'       => esc_html__( 'Pagination align', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SELECT,
						'condition' => [
										'pagination' => array( 'load_more' ),
				...
				$element->add_control(
					'pagination_style', array(
						'label'       => esc_html__( 'Pagination style', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SELECT,
						'condition' => [
										'pagination' => array( 'load_more' ),
				...
				$element->add_control(
					'pagination_color', array(
						'label'       => esc_html__( 'Pagination color', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SELECT,
						'condition' => [
										'pagination' => array( 'load_more' ),
				...
				$element->add_control(
					'pagination_hover', array(
						'label'       => esc_html__( 'Pagination hover', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SELECT,
						'condition' => [
										'pagination' => array( 'load_more' ),
			...
			if ( 'trx_sc_blogger' == $el_name  && 'section_sc_blogger_details' === $section_id ) {			
				$element->add_control( 
					'column_gap', array(
						'label'       => esc_html__( 'Gap between posts', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SELECT,
						'condition' => [
										'type' => [ 'default' ],
				...
				$element->add_control( 
					'full_size_image', array(
						...
						'label_off' => __( 'Off', 'kicker' ),
						'label_on' => __( 'On', 'kicker' ),
						'return_value' => '1',
						'default' => ''
					), array(
						'index' => 35
				...
				$element->add_control(
					'info_over_image', array(
						'label'       => esc_html__( 'Info over image', 'kicker' ),
						'description' => esc_html__('Show info over image for the post format Image', 'kicker'),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SELECT,
						'condition' => [
				...
				$element->add_control( 
					'post_title_tag', array(
						'label'       => esc_html__( 'Titles tag', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SELECT,
						'condition' => [
				...
				$element->add_control( 
					'post_title_over_tag', array(
						'label'       => esc_html__( 'Titles tag (over image)', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SELECT,
						'condition' => [
										'type' => [ 'default', 'wide' ],
				...
				$element->add_control( 
					'post_title_length', array(
						'label'       => esc_html__( 'Title length (in letters)', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::TEXT,
						'condition' => [
										'type' => [ 'default', 'wide', 'list', 'news' ],
				...
				$element->update_control(
					'on_plate', array(
						...
						'label' => esc_html__( 'On plate', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SWITCHER,
			...
			if ( 'trx_sc_layouts_menu' == $el_name  && 'section_sc_layouts_menu' === $section_id ) {
				$element->update_control(
					'animation_in',
					[
						'label' => __( 'Submenu animation in', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SELECT,
				...
				$element->update_control(
					'animation_out',
					[
						'label' => __( 'Submenu animation out', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SELECT,
			...
			if ( 'trx_widget_video' == $el_name  && 'section_sc_video' === $section_id ) {
				$element->add_control(
					'button_style', array(
						'label'       => esc_html__( 'Button style', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SELECT,
						'options' => array(
										'default' => esc_html__('Default', 'kicker'),
			...
			if ( 'trx_widget_video_list' == $el_name  && 'section_sc_video_list' === $section_id ) {
				...
				$element->add_control( 
					'content_style', array(
						'label'       => esc_html__( 'Content style', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SELECT,
						'options' => array(										
										'default' => esc_html__('Default', 'kicker'),
			...
			if ( 'trx_widget_video_list' == $el_name  && 'section_sc_video_list_controller' === $section_id ) {			
				$element->update_control( 
					'controller_pos', array(
						'label' => __( 'Position of the TOC', 'kicker' ),
						'label_block' => false,
						'type' => \Elementor\Controls_Manager::SELECT,
			...
			if ( 'trx_sc_layouts_search' == $el_name  && 'section_sc_layouts_search' === $section_id ) {
				...
				$element->update_control( 
					'post_types', array(
						'label' => __( 'Search in post types', 'kicker' ),
						'label_block' => false,
...
if ( ! function_exists( 'kicker_skin_trx_addons_sc_param_group_params' ) ) {
	add_filter( 'trx_addons_sc_param_group_params', 'kicker_skin_trx_addons_sc_param_group_params', 10, 2 );
	function kicker_skin_trx_addons_sc_param_group_params( $params, $sc ) {
		// Param 'hover_style'
			...
			if ( isset( $params[0]['name'] ) && isset( $params[0]['label'] ) ) {
				array_splice($params, 1, 0, array( 
					array(
						'name'        => 'hover_style',
						'type'        => \Elementor\Controls_Manager::SELECT,
						'label'       => esc_html__( 'Hover style', 'kicker' ),
						'label_block' => false,
						'condition' => [
							'type' => [ 'default' ],
						],
						'options'     => kicker_skin_get_list_sc_hover_styles(),
						'default'     => 'default',
					),
					array(
...
if ( ! function_exists( 'kicker_trx_addons_gb_map' ) ) {
	add_filter( 'trx_addons_gb_map', 'kicker_trx_addons_gb_map', 10, 2 );
	function kicker_trx_addons_gb_map($array, $sc) {	
		// Recent News	

<div class="front_page_section front_page_section_blog<?php
	$kicker_scheme = kicker_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $kicker_scheme ) && ! kicker_is_inherit( $kicker_scheme ) ) {
		echo ' scheme_' . esc_attr( $kicker_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( kicker_get_theme_option( 'front_page_blog_paddings' ) );
	if ( kicker_get_theme_option( 'front_page_blog_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$kicker_css      = '';
		$kicker_bg_image = kicker_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$kicker_anchor_icon = kicker_get_theme_option( 'front_page_blog_anchor_icon' );
	$kicker_anchor_text = kicker_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( kicker_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$kicker_css      = '';
			$kicker_bg_mask  = kicker_get_theme_option( 'front_page_blog_bg_mask' );
			$kicker_bg_color_type = kicker_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $kicker_bg_color_type ) {
				$kicker_bg_color = kicker_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$kicker_caption = kicker_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $kicker_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $kicker_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $kicker_caption, 'kicker_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$kicker_description = kicker_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $kicker_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $kicker_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $kicker_description ), 'kicker_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_blog_output">
				...
				if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
					...
					if ( ! kicker_exists_trx_addons() ) {
						...
						kicker_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
					}
				}
				?>
			</div>
